...
switch($action){
	...
	case 'valideCreation':{
		...
		$leLogin = htmlspecialchars($_POST['login']);
                ...
                $YEAR = strftime("%Y", strtotime($dateNaiss));
        ...
        if (empty($lePassword)==true){
         ...
         if (empty($dateNaiss)==true){
            echo 'La date n\'as pas été choisi<br/>';
            $rempli=false; 
        }
        ...
        if($rempli && $loginOk && $passwordOk && $coche==true){
                ...
                $executionOK = $pdo->creeMedecin($leNom,$lePrenom,$leLogin,$lePassword,$YEAR);
