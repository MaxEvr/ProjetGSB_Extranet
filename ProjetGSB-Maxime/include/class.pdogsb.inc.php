...
public static function DecryptMail($mail){
    $key= PdoGsb::recupKey();
    $nonce= PdoGsb::recupNonce();
    $decrypte = sodium_crypto_secretbox_open($mail, $nonce, $key);
    return $decrypte;    
}


//Fonctions qui récupère les infos du médecin à archiver
public static function RecupInfoMedecin($id){
    $pdo = PdoGsb::$monPdo;
    
    $sql = "SELECT nom,prenom,mail,dateNaissance,dateCreation,rpps,dateConsentement FROM medecin WHERE id = :lId";
...
public static function toJson($nom,$prenom,$mail,$dateNaiss,$dateCrea,$rpps,$dateConsentement){
    $file='css/MesInfos.odt';
    //$mail = PdoGsb::DecryptMail($mail);
    $json = json_encode(["nom" => $nom,
        "prenom" => $prenom,
        "mail" => $mail,
        "Date de naissance" => $dateNaiss,
        "Date de creation" => $dateCrea,
        "Numero de Medecin" => $rpps,
        "Date de Consentement" => $dateConsentement]);
...
public function creeMedecin($nom,$prenom,$email, $mdp,$dateNaiss)
{
    $mdp = hashPWD($mdp);
    $email = PdoGsb::hashMail($email);
    $pdoStatement = PdoGsb::$monPdo->prepare("INSERT INTO medecin(id,nom,prenom,mail,dateNaissance,motDePasse,dateCreation,dateConsentement) "
            . "VALUES (null, :leNom,:lePrenom ,:leMail,:laDateNaiss, :leMdp, now(),now())");
    ...
    $bv4 = $pdoStatement->bindValue(':laDateNaiss', $dateNaiss);
    $execution = $pdoStatement->execute();
    return $execution;
...
//Fonctions qui répcupère les infos du médecin à archiver
...
//Fonction qui ajoute le médecin à archiver dans les tables réspectives
...
//ajoute dans la bdd la date de connexion de l'utilisateur
...
//ajoute dans la bdd la date de deconnexion de l'utilisateur
